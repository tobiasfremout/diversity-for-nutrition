...
resolve_config_path <- function() {
  ...
  if (length(file_args) > 0) {
    ...
    candidates <- c(candidates, file.path(script_dir, "config.r"))
  }

  existing <- unique(candidates[file.exists(candidates)])
  if (length(existing) == 0) {
    stop("No se encontro config.r. Verifique que este archivo exista en el paquete desplegado.")
  }
  existing[1]
}
