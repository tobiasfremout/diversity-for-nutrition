...
generate_html_report <- function(nutr_table,
  ...
  captionLab <- trans_df$description
  HTML(captionLab, file = dirRepor, align = "left", row.names = FALSE)
  
  # update column names of nutr_table
  names(nutr_table) <- c(trans_df$species,
                         trans_df$family,
                         trans_df$growth_form,
                         trans_df$wild_or_cultivated,
                         trans_df$plant_part,
                         trans_df$food_group,
                         trans_df$use)
  
  # remove rownames of nutr_table
  rownames(nutr_table) <- NULL
