...
extract_suitable_species <- function(lon, lat, maps) {
  ...
  log_step("n04 [species_analysis]", "Extracting suitable species...")
...
filter_species <- function(nutr_dat,
  ...
  log_step("n05 [species_analysis]", "Filtering species: only suitable species...")
  
  # select only the suitable species
  j <- which(nutr_dat$species %in% suitable_species)
  nutr_dat <- nutr_dat[j,]
  
  log_step("n06 [species_analysis]", "Filtering species: edible parts...")
  
  # select only the species with edible parts selected by the user
  if(inputs$edible_parts_ID[1] != 1) {
    m <- match(inputs$edible_parts_ID, edible_parts$ID)
    edible_parts_sel <- edible_parts$plant_part[m]
    j <- which(nutr_dat$plant_part %in% edible_parts_sel)
    nutr_dat <- nutr_dat[j,]
  }
  
  log_step("n07 [species_analysis]", "Filtering species: food groups...")
  
  # select only the species with food groups selected by the user
  if(inputs$food_groups_ID[1] != 1) {
    m <- match(inputs$food_groups_ID, food_groups$ID)
    food_groups_sel <- food_groups$food_group[m]
    j <- which(nutr_dat$food_group %in% food_groups_sel)
    nutr_dat <- nutr_dat[j,]
  }
  
  log_step("n08 [species_analysis]", "Filtering species: growth forms...")
  
  # select only the species with growth forms selected by the user
  if(inputs$growth_forms_ID[1] != 1) {
    m <- match(inputs$growth_forms_ID, growth_forms$ID)
  ...
  # select only the species with growth forms selected by the user
  if(inputs$species_type_ID[1] != 1) {
    m <- match(inputs$species_type_ID, species_types$ID)
